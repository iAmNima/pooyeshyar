...
const Signup: React.FC = () => {
  ...
  const handleOrganizationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value === '') {
      setOrganizationError('سازمان الزامی است');
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (emailError || passwordError || nameError || organizationError) {
      return;
    }
    setLoading(true);
    try {
      ...
      toast.error('خطا در ثبت نام: ' + (error as Error).message);
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary transition-all duration-200 ease-in-out transform focus:scale-[1.02] ${nameError ? 'border-red-500' : 'border-gray-300'}`}
              required
            />
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary transition-all duration-200 ease-in-out transform focus:scale-[1.02] ${organizationError ? 'border-red-500' : 'border-gray-300'}`}
              required
            />
            {organizationError && <p className="text-red-500 text-sm mt-1">{organizationError}</p>}
          </div>
          <button
            type="submit"
            disabled={loading}
